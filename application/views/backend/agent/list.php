                             ...
                             <table class="m-datatable" id="html_table" width="100%">
                                 ...
                                 <tbody>
                                  ...
                                  <?php
                                    ...
                                    <tr>
                                      <td><?php echo $no;?></td>
                                      <td><?php echo $agent['first_name'];?> <?php echo $agent['last_name'];?></td>
                                      <td colspan="2"><?php echo $agent['email'];?></td>
                                      <td><?php echo $agent['phone'];?></td>
                                      <td><?php echo $agent['company'];?> </td>
                                      <td><?php echo $agent['position'];?></td> 
                                      <td><?php echo tgl_indo($agent['signup_date']);?></td>
                                      <td><?php if ($agent['step']==0) { echo 'waiting verification';} else { echo 'Verified'; }?></td>        
                                      <td>
                                         
                                          <?php if($approve){ ?>
                                                    <?php if($agent['status'] == 1) {?>
                                                            <a class="btn-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$agent['agent_id'];?>"><i class="la la-check-circle"></i></a> &nbsp; 
                                                    <?php } else { ?>
                                                            <a class="btn-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$agent['agent_id'];?>"><i class="la la-power-off"></i></a> &nbsp;
                                                    <?php } ?>
                                            <?php } ?>
                                            <?php if($delete){ ?>
                                                    <a class="btn-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $agent['first_name'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND.'/'.$controller;?>/delete/<?php echo $agent['agent_id'];?>';}"><span class="la la-trash"></span></a> &nbsp;
                                            <?php } ?>
                                      </td>
                                    </tr>
                                    <?php } ?>
