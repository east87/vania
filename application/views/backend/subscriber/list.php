                                      ...
                                      <td>
                                          ...
                                          <?php if($approve){ ?>
                                                    <?php if($subscriber['subscriber_status'] == 1) {?>
                                                            <a class="btn-outline-success btn-sm" title="Click to Inactive" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$subscriber['subscriber_id'];?>"><i class="la la-check-circle"></i></a> &nbsp; 
                                                    <?php } else { ?>
                                                            <a class="btn-outline-danger btn-sm" title="Click to Active" href="<?php echo BASE_URL_BACKEND."/".$controller."/active/".$subscriber['subscriber_id'];?>"><i class="la la-power-off"></i></a> &nbsp;
                                                    <?php } ?>
                                            <?php } ?>
                                            <?php if($delete){ ?>
                                                    <a class="btn-outline-danger btn-sm" title="Click to Delete" onclick="var answer = confirm('delete <?php echo $subscriber['email'];?> ?'); if (answer){window.location = '<?php echo BASE_URL_BACKEND.'/'.$controller;?>/delete/<?php echo $subscriber['subscriber_id'];?>';}"><span class="la la-trash"></span></a> &nbsp;
                                            <?php } ?>
                                      </td>
                                    </tr>
                                    <?php } ?>
